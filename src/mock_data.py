...
import pandas as pd                       # Importa pandas para estructurar la telemetría en tablas y exportar archivos CSV
...
def generate_mock_dataset():              # Declara la función principal que generará todo nuestro set de datos masivo
    ...
    num_samples = 2000                     # MÁXIMA CAPACIDAD: Multiplicamos por 20 el dataset para alimentar la ResNet18
    ...
    for i in range(num_samples):          # Inicia el bucle principal que iterará 2,000 veces para construir los registros
        ...
        data.append([                     # Inyecta toda la fila simulada dentro de la lista contenedora de memoria
            timestamp, panel_id, img_relative_path, 
            G, Tamb, V, I, p_real, severidad_visual
        ])

    columns = ['timestamp', 'panel_id', 'ruta_foto_actual', 'G', 'Tamb', 'V', 'I', 'P_real', 'severidad_visual'] # Define las cabeceras
