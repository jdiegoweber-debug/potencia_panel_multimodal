...
from torch.utils.data import DataLoader   # Importa el cargador para crear lotes aleatorios organizados y ordenados (Mini-Batches)
...
epochs = 20                           # Incrementamos a 20 épocas para darle tiempo a la ResNet18 de ajustar sus filtros desde cero
batch_size = 32                       # Subimos el tamaño del lote a 32 para suavizar y estabilizar las estimaciones de los gradientes
...
dataset = AsynchronousSolarDataset(csv_file=csv_path, img_dir=img_dir, transform=train_transform) # Instancia el cargador de datos unificado
...
print(f"[CONTROL] Dataset cargado con {len(dataset)} muestras.") # Muestra la confirmación de la carga masiva en pantalla
...
# 4. --- BUCLE PRINCIPAL DE ENTRENAMIENTO MASIVO ---
print("Iniciando entrenamiento del modelo Híbrido Multimodal...") 

for epoch in range(epochs):           # Recorre el ciclo de optimización a lo largo de las 20 épocas completas
    ...
    for images, op_data, p_real, visual_labels in train_loader: # Itera lote por lote sobre las 2,000 muestras distribuidas de a 32
    ...
    epoch_loss = running_loss / len(dataset) # Saca el promedio real de error dividiendo la acumulada por las 2,000 muestras totales
    print(f"Época [{epoch+1}/{epochs}] - Pérdida Total: {epoch_loss:.4f}") # Imprime la evolución del aprendizaje en la terminal
...
checkpoint_path = os.path.join(models_dir, "hybrid_solar_model.pth") # Define la ruta e identificación del archivo de pesos unificado
